...
$(document).ready(function() {
   ...
   IMP.init('imp13783325');  // 중요!!  아임포트에 가입시 부여받은 "가맹점 식별코드". 
   ...
   IMP.request_pay({
	   ...
	   */
		...
		*/
		<%--opener.location.href = "javascript:goCoinUpdate('<%= ctxPath%>', '${requestScope.userid}','${requestScope.productPrice}')"; --%>
			window.opener.goCoinUpdate('<%= ctxPath%>', '${requestScope.userid}','${requestScope.productPrice}');
		<%--$(opener.location).attr("href", "javascript:goCoinUpdate('<%= ctxPath%>', '${requestScope.userid}','${requestScope.productPrice}'"); --%>
			
			
		    self.close(); //팝업창 닫기(여기선 결체창)
